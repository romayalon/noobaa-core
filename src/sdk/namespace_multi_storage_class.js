...
class NamespaceMultiStorageClass {
    ...
    /**
     * Routes RestoreObject to the namespace that owns the object's storage class.
     * Resolves the owning namespace via read_object_md so that errors thrown by
     * the archive namespace (RestoreAlreadyInProgress, InvalidObjectState, etc.)
     * propagate correctly to the caller instead of being swallowed by _ns_get.
     * @param {object} params
     * @param {nb.ObjectSDK} object_sdk
     * @returns {Promise<object>}
     */
    async restore_object(params, object_sdk) {
        ...
        const ns = md.ns || this._ns_for_storage_class(md.storage_class);
        return ns.restore_object(params, object_sdk);
    }
