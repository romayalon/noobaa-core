...
mocha.describe('online upgrade S3 bucket operations tests', function() {
    ...
    mocha.it('create bucket - host is blocked for config dir updates - should fail', async function() {
    ...
    mocha.it('put bucket policy - host is blocked for config dir updates - should fail', async function() {
    ...
    mocha.it('delete bucket - host is blocked for config dir updates - should fail', async function() {
        await create_default_bucket(s3_client, bucket_defaults.name);
        await update_system_json(NC_CORETEST_CONFIG_FS, mock_old_config_dir_version);
    ...
    mocha.it('head bucket', async function() {
        await create_default_bucket(s3_client, bucket_defaults.name);
        await update_system_json(NC_CORETEST_CONFIG_FS, mock_old_config_dir_version);
        await s3_client.headBucket({ Bucket: bucket_defaults.name });
    }, TEST_TIMEOUT);

    mocha.it('PUT object - host is blocked for config dir updates', async function() {
        await create_default_bucket(s3_client, bucket_defaults.name);
        await update_system_json(NC_CORETEST_CONFIG_FS, mock_old_config_dir_version);
        await s3_client.putObject({ Bucket: bucket_defaults.name, Key: mock_obj_key, Body: mock_obj_body });
    }, TEST_TIMEOUT);

    mocha.it('GET object - host is blocked for config dir updates', async function() {
        await create_default_bucket(s3_client, bucket_defaults.name);
    ...
    mocha.it('HEAD object - host is blocked for config dir updates', async function() {
        await create_default_bucket(s3_client, bucket_defaults.name);
        await create_mock_object(s3_client, bucket_defaults.name, mock_obj_key, mock_obj_body);
        await update_system_json(NC_CORETEST_CONFIG_FS, mock_old_config_dir_version);
    ...
    mocha.it('DELETE object - host is blocked for config dir updates', async function() {
        await create_default_bucket(s3_client, bucket_defaults.name);
        await create_mock_object(s3_client, bucket_defaults.name, mock_obj_key, mock_obj_body);
        await update_system_json(NC_CORETEST_CONFIG_FS, mock_old_config_dir_version);
        await s3_client.deleteObject({ Bucket: bucket_defaults.name, Key: mock_obj_key});
